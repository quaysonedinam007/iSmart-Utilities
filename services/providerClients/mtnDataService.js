...
class MtnDataService {
  ...
  static async queryBundles(destination) {
    ...
    try {
      ...
      const url = hubtel.buildCommissionServiceUrl(MtnDataService.MTN_DATA_SERVICE_ID);
      ...
      console.error("[MtnDataService] queryBundles error:", error?.response?.data || error.message);
      return {
        success: false,
  ...
  static async buyBundle(payload, headers) {
    ...
    const reference = ClientReference || `MDAT-${Date.now()}`;
    ...
    try {
      ...
      const hubtelResponse = await hubtel.sendCommissionRequest(
        MtnDataService.MTN_DATA_SERVICE_ID,
        requestBody
      );

      if (!hubtelResponse.success) {
        await MtnDataService.#markFailure(reference, amountNumber, service_id, customer_id, hubtelResponse.message);
        return {
          success: false,
      ...
      console.error("[MtnDataService] buyBundle error:", error);
      await MtnDataService.#markFailure(reference, amountNumber, service_id, customer_id, error.message);
      return {
        success: false,
        status: 500,
        message: error.message || "Failed to initiate MTN data purchase"
      };
    }
  }
  ...
  static async handleCallback(callback) {
    ...
    try {
      ...
      if (success) {
        ...
        await MtnDataService.#refundTransaction(transaction, amount, description);
      }

      return { success: true, message: "Callback processed successfully" };
    } catch (error) {
      console.error("[MtnDataService] handleCallback error:", error);
  ...
  static async #refundTransaction(transaction, amount, description) {
    ...
    await prisma.transactions.create({
      data: {
        ...
        description: description || "Refund for failed MTN data purchase",
        status: "SUCCESS",
        created_at: new Date(),
        updated_at: new Date()
      }
  ...
  static async #markFailure(reference, amount, serviceId, customerId, errorMessage) {
    ...
    try {
      ...
      if (transaction) {
        await this.#refundTransaction(transaction, amount, errorMessage || "MTN data request failed");
      }
    } catch (error) {
      console.error("[MtnDataService] markFailure error:", error);
    }
  }
}

module.exports = MtnDataService;
