...
class ATDataService {
  static AT_DATA_SERVICE_ID = "06abd92da459428496967612463575ca";
  ...
  static async queryBundles(destination) {
    ...
    try {
      ...
      const url = hubtel.buildCommissionServiceUrl(ATDataService.AT_DATA_SERVICE_ID);
      ...
      console.error("[ATDataService] queryBundles error:", error?.response?.data || error.message);
      return {
        success: false,
  ...
  static async buyBundle(payload, headers) {
    ...
    const reference = ClientReference || `ADATA-${Date.now()}`;
    ...
    try {
      ...
      const hubtelResponse = await hubtel.sendCommissionRequest(
        ATDataService.AT_DATA_SERVICE_ID,
        requestBody
      );

      if (!hubtelResponse.success) {
        await ATDataService.#markFailure(reference, amountNumber, service_id, customer_id, hubtelResponse.message);
        return {
          success: false,
      ...
      console.error("[ATDataService] buyBundle error:", error);
      await ATDataService.#markFailure(reference, amountNumber, service_id, customer_id, error.message);
      return {
        success: false,
        status: 500,
        message: error.message || "Failed to initiate AT data purchase"
      };
    }
  }
  ...
  static async handleCallback(callback) {
    ...
    try {
      ...
      if (success) {
        ...
        await ATDataService.#refundTransaction(transaction, amount, description);
      }

      return { success: true, message: "Callback processed successfully" };
    } catch (error) {
      console.error("[ATDataService] handleCallback error:", error);
  ...
  static async #refundTransaction(transaction, amount, description) {
    ...
    await prisma.transactions.create({
      data: {
        ...
        description: description || "Refund for failed AT data purchase",
        status: "SUCCESS",
        created_at: new Date(),
        updated_at: new Date()
      }
  ...
  static async #markFailure(reference, amount, serviceId, customerId, errorMessage) { 
    ...
    try {
      ...
      if (transaction) {
        await this.#refundTransaction(transaction, amount, errorMessage || "AT   data request failed");
      }
    } catch (error) {
      console.error("[ATDataService] markFailure error:", error);
    }
  }
}

module.exports = ATDataService;
