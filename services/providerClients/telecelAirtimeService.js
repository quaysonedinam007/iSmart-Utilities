...
class TelecelAirtimeService {
  static TELECEL_AIRTIME_SERVICE_ID = "f4be83ad74c742e185224fdae1304800";

  /**
   * payload = { Destination, Amount, ClientReference, CallbackUrl }
   * headers = { customer_id, service_id }
   */
  static async buyTelecelAirtime(payload, headers) {
    console.log("[TelecelAirtimeService] headers:", headers);
    console.log("[TelecelAirtimeService] payload:", payload);
    ...
    console.log("[TelecelAirtimeService] wallet found for user:", customer_id, "=>", wallet);
    if (!wallet) {
      return {
        success: false,
        status: 404,
    ...
    console.log("[TelecelAirtimeService] balanceBefore:", balanceBefore);
    console.log("[TelecelAirtimeService] Amount:", Amount);
    if (balanceBefore < Amount) {
      console.log("[TelecelAirtimeService] Failing with Insufficient wallet balance");
      return {
        success: false,
        status: 400,
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const txn = await tx.transactions.create({
        data: {
          ...
          channel: "WALLET",
          status: "PENDING",
          description: "Telcel Airtime purchase",
          balance_before: balanceBefore,
          balance_after: balanceBefore - Amount,
          related_service_id: utility.id,
          service_id: service_id,
          system_reference: `AIRT-${Date.now()}`,
          user_type: "CUSTOMER",
    ...
    const hubtelResponse = await hubtel.sendCommissionRequest(
      TelecelAirtimeService.TELECEL_AIRTIME_SERVICE_ID,
      payload
    );
    const hubtelBody = hubtelResponse.data;
    const hubtelData = hubtelBody?. Data;
    console.log("[TelecelAirtimeService] hubtelData:", hubtelData);
    return {
      success: true,
      message: "Telecel airtime purchase initiated",
      data: {
...
module.exports =  TelecelAirtimeService;
