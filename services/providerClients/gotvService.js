...
class GotvService {
  ...
  static async buyGotv(payload, headers) {
    ...
    const reference = ClientReference || `GOTV-${Date.now()}`;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const txn = await tx.transactions.create({
        data: {
          ...
          channel: "WALLET",
          status: "PENDING",
          description: "GoTV subscription purchase",
          balance_before: balanceBefore,
          balance_after: balanceBefore - Amount,
          related_service_id: utility.id,
          service_id: service_id,
          system_reference: `CTG-${Date.now()}`,
          user_type: "CUSTOMER",
    ...
    const hubtelResponse = await hubtel.sendCommissionRequest(
      GotvService.GOTV_SERVICE_ID,
      payload
    );

    return {
      success: true,
      message: "GoTV subscription purchase initiated",
      data: {
      ...
      if (success) {
        ...
        await prisma.transactions.create({
          data: {
            ...
            description: `Refund for failed GoTV subsccription purchase: ${description}`,
            status: "SUCCESS",
            created_at: new Date(),
            updated_at: new Date()
          }
