...
class MtnBroadbandService {
  static MTN_BROADBAND_SERVICE_ID = "";
  ...
  static async queryBundles(destination) {
    ...
    try {
      ...
      const url = hubtel.buildCommissionServiceUrl(MtnBroadbandService.MTN_BROADBAND_SERVICE_ID);
      ...
      console.error("[MtnBroadbandService] queryBundles error:", error?.response?.data || error.message);
      return {
        success: false,
  ...
  static async buyBundle(payload, headers) {
    ...
    const reference = ClientReference || `MBRB-${Date.now()}`;
    ...
    try {
      ...
      const hubtelResponse = await hubtel.sendCommissionRequest(
        MtnBroadbandService.MTN_BROADBAND_SERVICE_ID,
        requestBody
      );

      if (!hubtelResponse.success) {
        await MtnBroadbandService.#markFailure(reference, amountNumber, service_id, customer_id, hubtelResponse.message);
        return {
          success: false,
      ...
      console.error("[MtnBroadbandService] buyBundle error:", error);
      await MtnBroadbandService.#markFailure(reference, amountNumber, service_id, customer_id, error.message);
      return {
        success: false,
        status: 500,
        message: error.message || "Failed to initiate MTN broadband purchase"
      };
    }
  }
  ...
  static async handleCallback(callback) {
    ...
    try {
      ...
      if (success) {
        ...
        await MtnBroadbandService.#refundTransaction(transaction, amount, description);
      }

      return { success: true, message: "Callback processed successfully" };
    } catch (error) {
      console.error("[MtnBroadbandService] handleCallback error:", error);
  ...
  static async #refundTransaction(transaction, amount, description) {
    ...
    await prisma.transactions.create({
      data: {
        ...
        description: description || "Refund for failed MTN broadband purchase",
        status: "SUCCESS",
        created_at: new Date(),
        updated_at: new Date()
      }
  ...
  static async #markFailure(reference, amount, serviceId, customerId, errorMessage) {
    ...
    try {
      ...
      if (transaction) {
        await this.#refundTransaction(transaction, amount, errorMessage || "MTN broadband request failed");
      }
    } catch (error) {
      console.error("[MtnBroadbandService] markFailure error:", error);
    }
  }
}

module.exports = MtnBroadbandService;
